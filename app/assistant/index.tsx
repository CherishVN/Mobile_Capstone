  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      if (res.success && res.orderId) {
        const payRes =
          paymentMethod === 'momo'
            ? await paymentService.createMoMo(res.orderId).catch(() => null)
            : await paymentService.createVNPay(res.orderId).catch(() => null)
        ...
        if (payRes?.success && payRes.paymentUrl) {
          setMessages((prev) => [
            ...prev,
            {
              id: `a-pay-${Date.now()}`,
              role: 'assistant',
              content: `Đơn đã tạo. Mở ${paymentMethod === 'momo' ? 'MoMo' : 'VNPay'} để thanh toán.`,
              createdAt: new Date().toISOString(),
            },
          ])
            ...
            <View key={p.id} style={[styles.productRowCard, s.checked && styles.productRowCardOn]}>
              ...
              <TouchableOpacity style={styles.productRowMid} onPress={() => openProduct(p.id)}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.productRowPrice}>{formatPrice(Number(p.basePrice))}</Text>
                </View>
              </TouchableOpacity>
